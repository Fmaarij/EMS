...
<body>
            ...
            <div class="card-body py-5 px-md-5">
              ...
              <label class="form-check-label" for="form2Example33">
                Subscribe to our newsletter
                            ...
                            <!-- Submit button -->
                            <button type="submit" name="u_log" class="btn btn-primary btn-block mb-4">
                                Log in
                            </button>

                            <button type="submit" class="btn btn-primary btn-block mb-4">
                                <a class="text-decoration-none btn-primary" href="register.php">Sign up</a>
                            </button>
    ...
    if (isset($_POST['u_log'])) {
        ...
        $sql = "SELECT * FROM users WHERE u_lastname='$u_lastname'";
        $result = mysqli_query($conn, $sql);

        if (mysqli_num_rows($result) > 0) {

            while ($user = mysqli_fetch_assoc($result)) {
                if ($u_lastname == $user['u_lastname'] && $u_password == $user["u_password"]) {
                    $_SESSION['u_lastname'] = $u_lastname;
                    // echo "You are logged in!";
                    // header("Location: dash.php"); #this is giving error cause header is already used before somewhere
                    echo "<script> location.replace('dash.php'); </script>";
                } else {
                    echo "<script>alert('Error: Lastname or password is not correct!');</script>";
                }
                // echo "id: " . $user['id'] . "Name: " . $user['name'] . "Lastname " . $user['lastname'] . "<br>";
            } #while ends here
        } else {
            echo "0 result found!";
        }
